...
pdf("../Figs/ri8X.pdf", width=9.75, height=6.5, pointsize=16, onefile=TRUE)
par(mar=rep(0.1,4),las=1,fg=fgcolor,col=fgcolor,col.axis=fgcolor,col.lab=fgcolor,
    bg=bgcolor,bty="n")
plot(0,0,xlim=c(-10,864),ylim=c(25,480),xaxt="n",yaxt="n",xlab="",ylab="",type="n")
...
for(i in 1:8) {
  ...
  rect(xloc+28,480,xloc+38,440+ifelse(i %% 2,0,30),col=color[i],border=color[i], lend=1, ljoin=1)
...
for(i in 1:4) {
  ...
  rect(xloc+ 9,400,xloc+19,360+ifelse(i %% 2,0,30),col=color[2*i],border=color[2*i], lend=1, ljoin=1)
...
file <- file.path(dir, "for_ri8X_fig.RData")
if(file.exists(file)) {
    load(file)
} else {
    f1 <- vector("list",4)
...
rect(xxloc[2]+ 9,320,xxloc[2]+19,280+30,col=color[7],border=color[7], lend=1, ljoin=1)
...
rect(xxloc+48,235,xxloc+58,195+30,col=color[5],border=color[5], lend=1, ljoin=1)
...
for(j in 2:ncol(temp)) {
  if(temp[2,j]!=5)
    rect(xxloc+48,195+temp[1,j]*mult,xxloc+58,195+temp[1,j-1]*mult+30,
         col=color[temp[2,j]],border=color[temp[2,j]], lend=1, ljoin=1)
}
...
rect(xxloc+48,150,xxloc+58,110+30,col=color[1],border=color[1], lend=1, ljoin=1)
...
for(j in 2:ncol(temp)) {
  if(temp[2,j]!=1)
    rect(xxloc+48,110+temp[1,j]*mult,xxloc+58,110+temp[1,j-1]*mult+30,
         col=color[temp[2,j]],border=color[temp[2,j]], lend=1, ljoin=1)
}
text(u[1],130,expression(G[4]),adj=c(0,0.5))
points(xxloc,130,pch=4,cex=1.3)
segments(xxloc,115,xxloc,95,lty=3)
segments(xxloc-39,95,xxloc+39,95, lty=3)
segments(xxloc+39*c(-1,1),95,xxloc+39*c(-1,1),80,lty=3)
arrows(xxloc+39*c(-1,1),80,xxloc+39*c(-1,1),75,len=0.1)
...
rect(xxloc+19*c(-1,1)-44,rep(65,2),xxloc+9*c(-1,1)-44,rep(a,2),col=color[1],border=color[1], lend=1, ljoin=1)
temp <- fa$mat
for(j in 2:ncol(fa$mat)) {
  if(temp[2,j]!=1)
    rect(xxloc+19*c(-1,1)-44,rep(a,2)+temp[1,j]*mult,xxloc+9*c(-1,1)-44,rep(a,2)+temp[1,j-1]*mult,
         col=color[temp[2,j]],border=color[temp[2,j]], lend=1, ljoin=1)
}


rect(xxloc+19*(-1)+44,65,xxloc+9*(-1)+44,a,col=color[1],border=color[1], lend=1, ljoin=1)
temp <- fa$mat
for(j in 2:ncol(fa$mat)) {
  if(temp[2,j]!=1)
    rect(xxloc+19*(-1)+44,a+temp[1,j]*mult,xxloc+9*(-1)+44,a+temp[1,j-1]*mult,
         col=color[temp[2,j]],border=color[temp[2,j]], lend=1, ljoin=1)
}

rect(xxloc+19+44,65,xxloc+9+44,a+30,col=color[8],border=color[8], lend=1, ljoin=1)
